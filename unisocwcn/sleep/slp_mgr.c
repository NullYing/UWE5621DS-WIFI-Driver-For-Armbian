...
void slp_mgr_drv_sleep(enum slp_subsys subsys, bool enable)
{
	...
	if (enable)
		...
		slp_mgr.active_module |= (BIT(subsys));
	/*
	 * MAINLINE FIX: never allow CP2 to enter sleep. The sleep/wake
	 * handshake relies on the OOB sdio_pub_int (bt/wl_wake_host) GPIO,
	 * which is not wired/functional on this mainline meson-g12a port.
	 * If the CP is allowed to sleep, the subsequent ap_wakeup_cp()
	 * handshake times out ("host wakeup fw cmd failed, ret=-1") and
	 * every command after the initial SYNC asserts. Keeping the CP
	 * awake is required for stable operation here.
	 */
	if (0 && slp_mgr.active_module == 0) {
