...
/*
 * MAINLINE FIX (meson-gx-mmc): reconstruct the valid byte length by walking
 * the self-describing SDIO public headers. Used as a fallback when the SPRD
 * SDMA tail descriptor (valid_len/dtbs in the last 8 bytes of the CMD53 read)
 * is not delivered by the mainline meson-gx-mmc controller even though the
 * packet payload itself is. Without this, fw cmd responses sitting in the rx
 * buffer would be silently dropped (SYNC_VERSION/get_cp2_version timeouts).
 */
static int sdiohal_rx_calc_validlen(char *buf, int max_len)
{
	struct sdio_puh_t *puh;
	int off = 0, acc = 0, guard = 0;

	while ((off + (int)SDIO_PUB_HEADER_SIZE) <= max_len &&
	       guard++ < MAX_CHAIN_NODE_NUM) {
		unsigned int len;

		puh = (struct sdio_puh_t *)(buf + off);
		if (puh->eof != 0 || puh->type == 0xF)
			break;
		len = puh->len;
		if (len == 0 ||
		    len > (MAX_PAC_SIZE - SDIO_PUB_HEADER_SIZE))
			break;
		acc += len;
		off += SDIO_PUB_HEADER_SIZE + SDIOHAL_ALIGN_4BYTE(len);
	}
...
read_again:
		...
		if (p_data->adma_rx_enable) {
			...
			/*
			 * MAINLINE: meson-gx-mmc does not deliver the SPRD SDMA
			 * tail descriptor (valid_len/dtbs) reliably. Per-boot the
			 * residual DMA pattern makes the trailer read back as either
			 * zero or stale garbage, and a garbage non-zero dtbs both
			 * (a) feeds a bogus valid_len into the parser and (b) shrinks
			 * the next adaptive read_len so a following response gets
			 * truncated -- this is the source of the per-boot cmd-rsp
			 * loss. So never trust the trailer: always recompute valid_len
			 * by walking the self-describing packet headers, always force
			 * the next read back to MAX_PAC_SIZE (rx_dtbs = 0), and drive
			 * draining purely from whether hdr-walk recovered packets.
			 */
