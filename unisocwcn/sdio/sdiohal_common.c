...
int sdiohal_tx_list_denq(struct sdiohal_list_t *data_list)
{
	...
	for (pos = list_head; pos->next != list_head; pos = pos->next) {
		...
		printk(KERN_ERR "DBG: TX dispatch chn=%d num=%d\n", channel, tx_list->node_num); if (sdiohal_ops && sdiohal_ops->pop_link) {
...
int sdiohal_rx_list_dispatch(void)
{
	...
	for (pos = list_head; pos->next != list_head; pos = pos->next) {
		...
		printk(KERN_ERR "DBG: RX dispatch chn=%d num=%d head_buf=%*phN\n", channel, rx_list->node_num, 16, rx_list->mbuf_head ? rx_list->mbuf_head->buf : NULL); if (sdiohal_ops && sdiohal_ops->pop_link) {
