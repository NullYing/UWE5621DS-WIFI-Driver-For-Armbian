		     ...
		     struct mbuf_t *tail, int num)
{
	...
	printk(KERN_ERR "DBG: mdbg_assert_read chn=%d num=%d\n", channel, num);
#if (CONFIG_CP2_ASSERT)
		     ...
		     struct mbuf_t *tail, int num)
{
	...
	printk(KERN_ERR "DBG: mdbg_at_cmd_read chn=%d num=%d head=%p buf=%p len=%d\n", channel, num, head, head ? head->buf : 0, head ? head->len : 0);
	data_length = mdbg_mbuf_get_datalength(head);
